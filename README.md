...
这是一个适合初学者继续扩展的 macOS 清理工具项目。

## 下载

普通用户请直接前往 GitHub `Releases` 下载编译好的桌面版：

- `macOS Cleaner-macOS.zip`

解压后双击 `macOS Cleaner.app` 即可使用，不需要本地安装 Python、PySide6 或重新打包。
...
如果你准备把项目开源到 GitHub，推荐这样发布：

- 仓库里放源码
- `Releases` 里放 `dist/macOS Cleaner-macOS.zip`
...
- 左侧导航
- 十个页面
- 全面检查 / 仪表盘 / 文件清理 / 图片管理 / 磁盘空间 / 软件缓存 / 开机启动 / 应用程序 / 内存管理 / 设置
- 菜单栏 helper 已接入，但在部分内屏菜单栏环境下可能仍存在显示异常，因此当前仍按实验性功能看待
