...
HELPER_BUILD_DIR="$PROJECT_ROOT/build/pyinstaller-menu"
...
rm -rf "$BUILD_DIR" "$HELPER_BUILD_DIR" "$DIST_DIR/macOS Cleaner" "$DIST_DIR/macOS Cleaner Menu" "$APP_PATH" "$HELPER_APP_PATH"
...
python3 -m PyInstaller \
  ...
  --clean \
  --distpath "$DIST_DIR" \
  --workpath "$HELPER_BUILD_DIR" \
  "$PROJECT_ROOT/packaging/macos_cleaner_menu.spec"
...
if [[ -d "$HELPER_APP" ]]; then
  /usr/bin/open -g -n "$HELPER_APP" --args --main-pid "$$" --main-app "$APP_BUNDLE" || true
fi
