...
def disable_startup_items(
    ...
    *,
    ...
    for item in selected_items or []:
        ...
        if log_callback:
            log_callback(f"正在处理启动项：{name or item_id}")
        try:
            if kind == "login_item":
                ...
                if log_callback:
                    log_callback(f"已关闭登录项：{name or item_id}")
                ...
                if log_callback:
                    log_callback(f"已关闭后台项目：{name or item_id}")
            else:
            ...
            if log_callback:
                log_callback(f"关闭失败：{name or item_id} | {exc}")
