...
def _iter_image_files(selected_roots: list[dict]) -> tuple[list[dict], list[dict], bool]:
    items: list[dict] = []
...
def _group_duplicates(items: list[dict]) -> list[dict]:
...
def _group_similar(items: list[dict]) -> list[dict]:
    groups: list[dict] = []
...
def scan_image_library(selected_roots: list[str] | None = None) -> dict:
    ...
    items, root_summaries, truncated = _iter_image_files(configs)
    ...
    duplicate_groups = _group_duplicates(items)
    similar_groups = _group_similar(items)
