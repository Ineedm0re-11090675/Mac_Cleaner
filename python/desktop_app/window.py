...
class DashboardPage(QWidget):
    def __init__(self, bridge: CleanerBridge, parent: QWidget | None = None) -> None:
        ...
        self._history_cpu: deque[float] = deque(maxlen=60)
        self._history_memory: deque[float] = deque(maxlen=60)
        self._history_disk: deque[float] = deque(maxlen=60)
        self._history_network: deque[float] = deque(maxlen=60)
        ...
        self.tip_label = QLabel("提示：CPU / 内存 / 网络每秒刷新；Top 进程约每 2 秒刷新；磁盘使用率约每 5 秒刷新。")
    ...
    def on_activated(self) -> None:
        self._tick_count = 0
        self._refresh_metrics(force_processes=True, force_disk=True)
    ...
    def _refresh_metrics(self, *, force_processes: bool = False, force_disk: bool = False) -> None:
        self._tick_count += 1
        include_processes = force_processes or self._sample is None or self._tick_count % 2 == 0
        include_disk = force_disk or self._sample is None or self._tick_count % 5 == 0
        try:
            self._sample = sample_dashboard(
                self._sample,
                process_limit=6,
            ...
            self.summary_label.setText(f"仪表盘刷新失败：{exc}")
            self.updated_label.setText("请稍后重试，或切换页面后再回来。")
            return
        self._render_sample(self._sample)
    ...
    def _render_sample(self, sample: dict) -> None:
        ...
        self.updated_label.setText(
            ...
            f"趋势窗口 {len(self._history_cpu)} 秒"
        )
    ...
    def run_quick_cache_cleanup(self) -> None:
    ...
    def run_quick_memory_reclaim(self) -> None:
        if self._action_task:
            return
        ...
        def _success(result: dict, logs: str) -> None:
            ...
            self._refresh_metrics(force_processes=True, force_disk=False)
...
class CleanerDesktopWindow(QMainWindow):
    ...
    def run_tray_quick_cache_cleanup(self) -> None:
        ...
        if isinstance(dashboard, DashboardPage):
            QTimer.singleShot(180, dashboard.run_quick_cache_cleanup)

    def run_tray_quick_memory_reclaim(self) -> None:
        self.open_dashboard()
        dashboard = self.pages.get("dashboard")
        if isinstance(dashboard, DashboardPage):
            QTimer.singleShot(180, dashboard.run_quick_memory_reclaim)
