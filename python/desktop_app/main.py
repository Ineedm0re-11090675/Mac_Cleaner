...
def is_menu_helper_running(helper_exec: Path | None = None) -> bool:
    if helper_exec is not None:
        try:
            completed = subprocess.run(
                ["pgrep", "-f", str(helper_exec)],
                check=False,
                capture_output=True,
                text=True,
            )
...
def main() -> int:
    ...
    def resolve_helper_app_path() -> Path | None:
        ...
        helper_path = Path(__file__).resolve().parents[2] / "dist" / "macOS Cleaner Menu.app"
        return helper_path if helper_path.exists() else None

    def resolve_helper_executable_path() -> Path | None:
        if not getattr(sys, "frozen", False):
            native_helper = Path(__file__).resolve().parents[2] / "build" / "macOS Cleaner Menu"
            if native_helper.exists():
        ...
        if getattr(sys, "frozen", False):
            ...
            try:
                ...
                subprocess.Popen(
                    ...
                    cwd=str(helper_exec.parent),
                    stdout=stdout_handle,
                    stderr=stderr_handle,
                    start_new_session=True,
                )
                append_runtime_diag(f"launch_menu_helper started bundled helper executable: {helper_exec}")
                return True
            except OSError as exc:
                append_runtime_diag(f"launch_menu_helper failed to start bundled helper executable: {exc}")
                return False
        try:
            stdout_path = RUNTIME_DIR / "tray-helper.stdout.log"
            stderr_path = RUNTIME_DIR / "tray-helper.stderr.log"
            ...
            if helper_exec is not None:
                ...
                append_runtime_diag(f"launch_menu_helper started native dev helper pid={process.pid}")
                return True
            ...
            app._macos_cleaner_helper_process = process  # type: ignore[attr-defined]
            append_runtime_diag(f"launch_menu_helper started fallback python helper pid={process.pid}")
            return True
        except OSError as exc:
    ...
    helper_command_timer.setInterval(120)
    ...
    helper_launch_attempts = {"count": 0}

    def ensure_helper_running() -> None:
        helper_exec = resolve_helper_executable_path()
        if is_menu_helper_running(helper_exec):
            append_runtime_diag("menu helper is running")
            window.set_tray_available(True)
            return

        helper_launch_attempts["count"] += 1
        attempted_by_wrapper = os.environ.get("MACOS_CLEANER_TRAY_BOOTSTRAPPED") == "1"
        if attempted_by_wrapper and helper_launch_attempts["count"] == 1:
            append_runtime_diag("wrapper attempted helper bootstrap, but helper is not running; retrying from main app")
        launched_helper = launch_menu_helper()
        window.set_tray_available(launched_helper)
        if helper_launch_attempts["count"] < 3:
            QTimer.singleShot(1500, ensure_helper_running)

    QTimer.singleShot(750, ensure_helper_running)
