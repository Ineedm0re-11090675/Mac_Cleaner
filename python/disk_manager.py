...
def _walk_size(path: Path, top_files_heap: list[tuple[int, str, dict]], source_root: dict) -> tuple[int, int]:
    if path.is_symlink():
        return 0, 0
    ...
    try:
        with os.scandir(path) as iterator:
            for entry in iterator:
                ...
                size, file_count = _walk_size(entry_path, top_files_heap, source_root)
                total_size += size
...
def scan_disk_usage(selected_roots: list[str] | None = None) -> dict:
    ...
    for config in root_configs:
        ...
        for child in children:
            if child.name.startswith("."):
                continue
            size_bytes, file_count = _walk_size(child, top_files_heap, config)
            if size_bytes <= 0:
                continue
