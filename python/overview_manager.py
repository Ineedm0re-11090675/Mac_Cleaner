...
def _file_card(bridge: CleanerBridge, *, task_context=None, log_callback=None, cancel_check=None) -> dict:
    if log_callback:
        log_callback("正在检查文件清理模块。")
    raise_if_cancelled(cancel_check)
    result = bridge.scan(["user_cache", "user_logs", "trash"], task_context=task_context)
    raise_if_cancelled(cancel_check)
    categories = {item.get("key"): item for item in result.get("categories", [])}
...
def _cache_card(bridge: CleanerBridge, *, task_context=None, log_callback=None, cancel_check=None) -> dict:
    if log_callback:
        log_callback("正在检查软件缓存模块。")
    raise_if_cancelled(cancel_check)
    result = bridge.scan_app_caches([], task_context=task_context)
    raise_if_cancelled(cancel_check)
...
def _startup_card(*, log_callback=None, cancel_check=None) -> dict:
    if log_callback:
        log_callback("正在检查开机启动模块。")
    raise_if_cancelled(cancel_check)
    try:
...
def _memory_card(*, log_callback=None, cancel_check=None) -> dict:
    if log_callback:
        log_callback("正在检查内存管理模块。")
    raise_if_cancelled(cancel_check)
...
def _applications_card(*, log_callback=None, cancel_check=None) -> dict:
    if log_callback:
        log_callback("正在检查应用程序模块。")
    raise_if_cancelled(cancel_check)
...
def _images_card(*, log_callback=None, cancel_check=None) -> dict:
    if log_callback:
        log_callback("正在检查图片管理模块。")
    raise_if_cancelled(cancel_check)
    result = scan_image_library(["desktop", "downloads", "pictures"], log_callback=log_callback, cancel_check=cancel_check)
    raise_if_cancelled(cancel_check)
...
def _space_card(*, log_callback=None, cancel_check=None) -> dict:
    if log_callback:
        log_callback("正在检查磁盘空间模块。")
    raise_if_cancelled(cancel_check)
    result = scan_disk_usage(["desktop", "downloads", "documents", "library"], log_callback=log_callback, cancel_check=cancel_check)
    raise_if_cancelled(cancel_check)
...
def scan_overview(
    bridge: CleanerBridge | None = None,
    ...
    *,
    ...
    steps = [
        ("files", "正在检查文件清理...", lambda: _file_card(bridge, task_context=task_context, log_callback=log_callback, cancel_check=cancel_check)),
        ("images", "正在检查图片管理...", lambda: _images_card(log_callback=log_callback, cancel_check=cancel_check)),
        ("space", "正在检查磁盘空间...", lambda: _space_card(log_callback=log_callback, cancel_check=cancel_check)),
        ("caches", "正在检查软件缓存...", lambda: _cache_card(bridge, task_context=task_context, log_callback=log_callback, cancel_check=cancel_check)),
        ("startup", "正在检查开机启动...", lambda: _startup_card(log_callback=log_callback, cancel_check=cancel_check)),
        ("memory", "正在检查内存管理...", lambda: _memory_card(log_callback=log_callback, cancel_check=cancel_check)),
        ("applications", "正在检查应用程序...", lambda: _applications_card(log_callback=log_callback, cancel_check=cancel_check)),
    ]
    cards = []
    total = len(steps)
...
def execute_overview_actions(
    ...
    *,
    ...
    total_steps = sum(bool(group) for group in [clean_categories, clean_file_paths, startup_items, process_items, residual_items])
    current_step = 0
    ...
    if clean_categories:
        ...
        raise_if_cancelled(cancel_check)
        if progress_callback:
            progress_callback(current_step - 1, total_steps, "正在执行基础清理...")
        if log_callback:
            log_callback("正在执行基础清理。")
        result = bridge.clean(clean_categories, dry_run=False, task_context=task_context)
        ...
        if log_callback:
            log_callback(f"基础清理完成：成功 {success}，失败 {failed}。")

    if clean_file_paths:
        current_step += 1
        raise_if_cancelled(cancel_check)
        if progress_callback:
            progress_callback(current_step - 1, total_steps, "正在删除所选文件...")
        if log_callback:
            log_callback("正在删除总览中的文件类项目。")
        result = bridge.clean_files(clean_file_paths, dry_run=False, task_context=task_context)
        ...
        if log_callback:
            log_callback(f"文件类处理完成：成功 {success}，失败 {failed}。")

    if startup_items:
        current_step += 1
        raise_if_cancelled(cancel_check)
        if progress_callback:
            progress_callback(current_step - 1, total_steps, "正在关闭启动项...")
        if log_callback:
            log_callback("正在处理总览中的启动项。")
        result = disable_startup_items(startup_items, log_callback=log_callback, cancel_check=cancel_check)
        ...
        if log_callback:
            log_callback(f"启动项处理完成：成功 {success}，失败 {failed}。")

    if process_items:
        current_step += 1
        raise_if_cancelled(cancel_check)
        if progress_callback:
            progress_callback(current_step - 1, total_steps, "正在结束高占用进程...")
        if log_callback:
            log_callback("正在处理总览中的内存管理项目。")
        result = terminate_processes(process_items, log_callback=log_callback, cancel_check=cancel_check)
        ...
        if log_callback:
            log_callback(f"内存管理处理完成：成功 {success}，失败 {failed}。")

    if residual_items:
        current_step += 1
        raise_if_cancelled(cancel_check)
        if progress_callback:
            progress_callback(current_step - 1, total_steps, "正在删除应用残留...")
        if log_callback:
            log_callback("正在处理总览中的应用残留项目。")
        result = clean_app_residuals(residual_items, log_callback=log_callback, cancel_check=cancel_check)
        ...
        if log_callback:
            log_callback(f"应用残留处理完成：成功 {success}，失败 {failed}。")

    if progress_callback and total_steps:
        progress_callback(total_steps, total_steps, "正在汇总处理结果...")
    if log_callback:
        log_callback("正在汇总总览处理结果。")
